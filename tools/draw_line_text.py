...
def draw_horizontal_lines(data, image_folder, output_folder):
    # 获取表格图片的信息
    image_name = data['tableLabelList'][0]['tableImages'][0]['md5']
    ...
    for cell in data['tableLabelList'][0]['cells']:
...
def process_json_files(json_folder, image_folder, output_folder):
    ...
    for filename in os.listdir(json_folder):
        if filename.endswith('.json'):
            ...
            # 处理 JSON 数据
            draw_horizontal_lines(data, image_folder, output_folder)
